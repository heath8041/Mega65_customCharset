#import "../Mega65_System/System_Macros.s"   //include some of the the constants aka lables for VIC and SID so we don't need to do that manually every time.
...
everyFrame:
    ...
    jsr fineScroll
    jsr colorCycle          // slide the color gradient to the left
    jmp everyFrame
...
!:
  ...
  lda VIC.SCREEN + OFFSET + 80 , x
  sta VIC.SCREEN + OFFSET + 80 - 1 , x
  inx
  cpx #80
  bne !-
...
fillColorOnline:
  ldy #80
!:
  jsr getGradientColor            //loads a register with the next color
  sta VIC.COLOR_RAM + OFFSET, y   //store it into vic color ram
  sta VIC.COLOR_RAM + OFFSET + 80, y
  dey
  cpy #$ff
  bne !-
...
//Clear the VIC.SCREEN in 250 character blocks
//clobbers X and A
clearScreen:
  ldx #250
...
#import "charset1.asm"     // load the charset_1.asm into this project, this charset is created using https://petscii.krissz.hu/ editor. Then exported to assembly and made to work with kickassembler